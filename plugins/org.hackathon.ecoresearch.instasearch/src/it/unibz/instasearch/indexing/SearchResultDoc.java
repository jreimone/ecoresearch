...
public class SearchResultDoc {
	...
	private double[] termScoreVector;
	private float score;
	 ...
	 * @throws IOException
	 */
	private double[] getTermScoreVector()  throws IOException
	{
	 ...
	 * @throws IOException
	 */
	public double[] getTermScoreVector(Collection<String> terms) throws IOException
	{
		double[] allTermScoreVect = getTermScoreVector();
		double[] termScoreVect = new double[allTermScoreVect.length]; 
	...
	private double[] createTermScoreVector(TermFreqVector vect, IndexReader reader) throws IOException
	{		
		if( vect == null )
			return new double[0];
		...
		for(int i = 0; i < terms.length; i++) {
			...
			double termFreq = sim.tf( termFrequencies[i] );
