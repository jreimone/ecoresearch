...
class ResultContentProvider implements ITreeContentProvider {
	...
	public static final int MAX_LINES_TO_PROCESS = 10000; // max nr of lines to read from files (prevent slowdown on huge files)
	...
	public Object[] getChildren(Object parent) 
	{
		if( parent instanceof SearchResultDoc ) {
			...
			try {
				matches = getMatchLines(doc, true);
				if( matches != null )
	 ...
	 * @throws Exception
	 */
	List<MatchLine> getMatchLines(SearchResultDoc doc, boolean limit) throws Exception {
		...
		while ( (line = lineReader.readLine()) != null ) {
			...
			double[] lineTermScoreVector = doc.getTermScoreVector(lineTerms.keySet());
			double[] matchedTermScoreVector = doc.getTermScoreVector(matchedTerms);
	...
	{
		double[] vect1 = lineMatches1.getScoreVector();
		double[] vect2 = lineMatches2.getScoreVector();
	...
	class MatchLine implements Comparable<MatchLine> {
		...
		private MatchLine(SearchResultDoc doc, String lineText, int lineNumber, Set<String> matchedTerms, double[] termScoreVector, double[] matchedTermScoreVector) throws IOException {
			this.doc = doc;
		 ...
		 * @return
		 */
		private double getMagnitude(double[] vect)
		{
			double magnitude = 0;
