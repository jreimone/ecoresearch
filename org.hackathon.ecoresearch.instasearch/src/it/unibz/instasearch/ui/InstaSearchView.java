...
public class InstaSearchView extends ViewPart implements ModifyListener, ILogListener, ITreeViewerListener, IPropertyChangeListener {
	...
	private void openSelection() throws Exception {
		...
		if(obj instanceof SearchResultDoc) {
			...
			EObject selectedElement = searchInModel(doc.getFile());
			IOpenEditorHandler.INSTANCE.openEditorBySearchResultSelection(selectedElement);
	...
	private EObject searchInModel(IFile file) {
		...
		while (i.hasNext()) {
			Object elementObject = i.next();
			if (elementObject instanceof ENamedElement) {
